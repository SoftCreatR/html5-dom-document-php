...
class HTML5DOMDocument extends DOMDocument
{
    ...
    public function __construct(string $version = '1.0', string $encoding = '')
    {
        ...
        $this->registerNodeClass('DOMElement', HTML5DOMElement::class);
    }
    ...
    public function loadHTML(string $source, int $options = 0): bool
    {
        ...
        if (isset($matches[0])) {
            ...
            foreach ($matches[0] as $match) {
                if (\substr($match, -2, 1) !== '/') { // check if ends with />
                    $source = \str_replace(
                        $match,
                        $match . '<![CDATA[-html5-dom-document-internal-cdata',
                        $source
                    ); // Add CDATA after the open tag
                }
            }
        }

        // Add CDATA before the end tag
        $source = \str_replace(
            ['</script>', '<![CDATA[-html5-dom-document-internal-cdata-html5-dom-document-internal-cdata]]>'],
            ['-html5-dom-document-internal-cdata]]></script>', ''],
            $source
        ...
        \preg_match_all(
            '/<!\[CDATA\[-html5-dom-document-internal-cdata.*?-html5-dom-document-internal-cdata]]>/s',
            $source,
            $matches
        );
        ...
        if (isset($matches[0])) {
            ...
            foreach ($matches[0] as $match) {
                if (\str_contains($match, '</')) { // check if contains </
                    $source = \str_replace(
                        $match,
                        \str_replace('</', '<-html5-dom-document-internal-cdata-endtagfix/', $match),
                        $source
                    );
                }
            }
        ...
        $autoAddHtmlAndBodyTags = !\defined('LIBXML_HTML_NOIMPLIED')
            || ($options & LIBXML_HTML_NOIMPLIED) === 0;
        $autoAddDoctype = !\defined('LIBXML_HTML_NODEFDTD')
            || ($options & LIBXML_HTML_NODEFDTD) === 0;
        ...
        $source = \preg_replace(
            '/&([a-zA-Z]*);/',
            'html5-dom-document-internal-entity1-$1-end',
            $source
        );

        $source = \preg_replace(
            '/&#(\d*);/',
            'html5-dom-document-internal-entity2-$1-end',
            $source
        );

        // Add DOCTYPE if missing
        if ($autoAddDoctype && \stripos($source, '<!DOCTYPE') !== 0) {
            $source = "<!DOCTYPE html>\n" . $source;
        }
        ...
        /** @var HTML5DOMElement|null $metaTagElement */
        $metaTagElement = $this->getElementsByTagName('meta')->item(0);

        if (
            $metaTagElement !== null
            && $metaTagElement->getAttribute('data-html5-dom-document-internal-attribute') === 'charset-meta'
        ) {
            ...
            if (
                ...
                && (
                    ...
                    || (
                        $headElement->childNodes->length === 1
                        && $headElement->firstChild instanceof DOMText
                    )
                )
            ) {
        ...
        if (!$allowDuplicateIDs) {
            ...
            if (!empty($matches[2]) && \max(\array_count_values($matches[2])) > 1) {
                ...
                $walkChildren = static function ($element) use (&$walkChildren, &$elementIDs) {
                                ...
                                if ($id !== '') {
                                    if (isset($elementIDs[$id])) {
                                        throw new RuntimeException(
                                            'A DOM node with an ID value "' . $id . '" already exists! '
                                            . 'Pass the HTML5DOMDocument::ALLOW_DUPLICATE_IDS option to disable this check.'
                                        );
                                    }
    ...
    private function addHtmlElementIfMissing(): bool
    {
        if ($this->getElementsByTagName('html')->length === 0) {
            $this->appendChild(self::$newObjectsCache['htmlelement'] ?? new DOMElement('html'));
    ...
    private function addBodyElementIfMissing(): bool
    {
        if ($this->getElementsByTagName('body')->length === 0) {
            ...
            $this->getElementsByTagName('html')->item(0)?->appendChild(
                clone self::$newObjectsCache['bodyelement']
            );
    ...
    public function saveHTML(?DOMNode $node = null): string
    {
        ...
        if ($nodeMode) {
            if (!isset(self::$newObjectsCache['html5domdocument'])) {
                self::$newObjectsCache['html5domdocument'] = new self();
            }
            ...
            if ($node->nodeName === 'html') {
                ...
                // remove the DOCTYPE + the new line + html + body or head tags
                $html = \substr($html, 28, -14);
            }
            ...
            if (\str_contains($html, 'html5-dom-document-internal-entity')) {
     ...
     * @throws DOMException
     */
    public function saveHTMLFile(string $filename): false|int
    {
     ...
     */
    public function querySelector(string $selector): ?HTML5DOMElement
    {
     ...
     * @return bool|HTML5DOMElement A new DOMElement that must be set in the place where the new body will be inserted.
     *
     * @throws DOMException
     * @throws Exception
     */
    public function createInsertTarget(string $name): bool|HTML5DOMElement
    {
        if (!$this->loaded) {
            $this->loadHTML('');
        }
     ...
     * @throws Exception
     */
    public function insertHTML(string $source, string $target = 'beforeBodyEnd'): void
    {
     ...
     * @throws Exception
     */
    public function insertHTMLMulti(array $sources): void
    {
        if (!$this->loaded) {
            $this->loadHTML('');
        }
        ...
        $copyAttributes = static function ($sourceNode, $targetNode) {
        ...
        foreach ($sources as $sourceData) {
            if (!isset($sourceData['source'])) {
                throw new RuntimeException('Missing source key');
            }

            $source = $sourceData['source'];
            $target = $sourceData['target'] ?? 'beforeBodyEnd';
            ...
            if ($htmlElement !== null && $htmlElement->attributes->length > 0) {
     ...
     */
    public function modify(int $modifications = 0): void
    {
            ...
            if ($fixDuplicateStyles) {
                ...
                if ($styles->length > 0) {
                    ...
                    foreach ($styles as $style) {
                        ...
                        $innerHTML = \trim($style->innerHTML);

                        if (!\in_array($innerHTML, $list, true)) {
                            $list[] = $innerHTML;
                        } else {
